...
//Registra un log
router.post('/regLog', (req, res) => {
    const { idPropi, fechaIng, horaIng, usuario, dest } = req.body
    let sql = `insert into logs( idPropi, fechaIng, horaIng, usuario, dest ) values( '${idPropi}', '${fechaIng}', '${horaIng}', '${usuario}', '${dest}' )`
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
        else {
            res.json({ status: 'Log Agregado agregado' })
        }
    })
})
//Registra  participante
router.post('/addParticipante', (req, res) => {
    const { userPart, numReg, Usuario_ci, Participantecol } = req.body
    let sql = `insert into participante( userPart, numReg, Usuario_ci , Participantecol ) values( '${userPart}', '${numReg}', '${Usuario_ci}' , '${Participantecol}' )`
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
        else {
            res.json({ status: 'relacion comenta actividad agregado' })
...
//Registrar relacion Participante inscribe Evento
router.post('/addInscribe', (req, res) => {
    const { Evento_idEvento, Participante_userPart, asistencia, estado, fechaInscripcion } = req.body
    let sql = `insert into inscribe( Evento_idEvento, Participante_userPart, asistencia , estado, fechaInscripcion ) values( '${Evento_idEvento}', '${Participante_userPart}', '${asistencia}' , '${estado}', '${fechaInscripcion}' )`
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
        else {
            res.json({ status: 'relacion participante inscribe evento agregado' })
        }
    })
})
//Registrar relacion participante participa en actividad
router.post('/addParticipa', (req, res) => {
    const { Evento_idEvento, Participante_userPart, asistencia, estado, fechaInscripcion } = req.body
    let sql = `insert into inscribe( Evento_idEvento, Participante_userPart, asistencia , estado, fechaInscripcion ) values( '${Evento_idEvento}', '${Participante_userPart}', '${asistencia}' , '${estado}', '${fechaInscripcion}' )`
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
...
router.post('/addLogo', (req, res) => {
    const { logo } = req.body
    let sql = `insert into informacion( logo ) values( '${logo}' )`
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
        else {
            res.json({ status: 'logo agregado' })
        }
    })
})
...
router.get('/actividades/:idEvento', (req, res) => {
    ...
    const sql = `SELECT a.idActividad, a.nomAct, a.horaAct, a.fechaAct, a.desAct, e.userExpo, e.DescExpo, u.nombre, u.apellido, u.tituloProf, u.telefono, u.correo, u.nacionalidad, u.foto, u.ci 
                 FROM actividad a 
...
router.get('/eventosInscrito/:usuario_ci', (req, res) => {
    ...
    let sql = `SELECT e.idEvento, e.nomEv, e.descEv, e.tipoEv, e.fechaIni, e.fechaFin, e.horaIni, e.horaFin, e.portada, e.direccionEv, i.estado, i.nroIncr
               FROM evento e 
...
// Obtiene la tabla de logs disponible
router.get('/verLogs', (req, res) => {
...
router.get('/controlador/:ci', (req, res) => {
    const { ci } = req.params;
    const query = 'SELECT controlador.* FROM controlador WHERE controlador.Usuario_ci = ?';
    conexion.query(query, [ci], (error, results) => {
        if (error) throw error;
        res.json(results[0]);
    });
...
router.get('/actividadesControladas/:Usuario_ci', (req, res) => {
    const { Usuario_ci } = req.params;
    const query = `
      SELECT evento.*, actividad.*
      FROM evento
      INNER JOIN controla ON evento.idEvento = controla.Evento_idEvento
      INNER JOIN controlador ON controla.Controlador_userCont = controlador.userCont
      INNER JOIN actividad ON actividad.Evento_idEvento = evento.idEvento
      WHERE controlador.Usuario_ci = ?
    `;
    conexion.query(query, [Usuario_ci], (error, results) => {
        if (error) throw error;
        res.json(results);
    });
});
...
router.get('/participantesActividad/:idActividad', (req, res) => {
    const { idActividad } = req.params;
    const query = `SELECT usuario.*
                   FROM usuario
                   INNER JOIN participante ON usuario.ci = participante.Usuario_ci
                   INNER JOIN participa ON participante.userPart = participa.Participante_userPart
                   WHERE participa.Actividad_idActividad = ?`;
    conexion.query(query, [idActividad], (error, results) => {
        if (error) throw error;
        res.json(results);
    });
});
//Devuelve todos las actividades en lso que un usario participa
router.get('/actividadesUsuario/:ci', (req, res) => {
    const { ci } = req.params;
    const query = `SELECT actividad.*
                   FROM actividad
                   INNER JOIN participa ON actividad.idActividad = participa.Actividad_idActividad
                   INNER JOIN participante ON participante.userPart = participa.Participante_userPart
                   INNER JOIN usuario ON participante.Usuario_ci = usuario.ci
                   WHERE usuario.ci = ?`;
    conexion.query(query, [ci], (error, results) => {
        if (error) throw error;
...
//Verifica si un usuario participa en una actividad
router.get('/verificarParticipacion/:ci/:idActividad', (req, res) => {
    const { ci, idActividad } = req.params;
    const query = `SELECT participa.*
                   FROM participa
                   INNER JOIN participante ON participa.Participante_userPart = participante.userPart
                   WHERE participante.Usuario_ci = ? AND participa.Actividad_idActividad = ?`;
    conexion.query(query, [ci, idActividad], (error, results) => {
        if (error) throw error;
        if (results.length > 0) {
            res.json({ participa: true });
        } else {
            res.json({ participa: false });
        }
    });
});
...
//Devuelve todos los usuarios inscritos en un evento
router.get('/usuariosEvento/:idEvento', (req, res) => {
    const { idEvento } = req.params;
    const query = `SELECT usuario.*, inscribe.nroIncr, inscribe.asistencia, inscribe.estado, inscribe.fechaInscripcion
                   FROM usuario
                   INNER JOIN participante ON usuario.ci = participante.Usuario_ci
                   INNER JOIN inscribe ON participante.userPart = inscribe.Participante_userPart
                   INNER JOIN evento ON inscribe.Evento_idEvento = evento.idEvento
                   WHERE evento.idEvento = ? AND inscribe.estado = 'inscrito'`;
    conexion.query(query, [idEvento], (error, results) => {
        if (error) throw error;
        res.json(results);
    });
...
//Devuelve todos los usuarios reservados en un evento
router.get('/usuariosReserva/:idEvento', (req, res) => {
    const { idEvento } = req.params;
    const query = `SELECT usuario.*, inscribe.nroIncr, inscribe.asistencia, inscribe.estado, inscribe.fechaInscripcion
                   FROM usuario
                   INNER JOIN participante ON usuario.ci = participante.Usuario_ci
                   INNER JOIN inscribe ON participante.userPart = inscribe.Participante_userPart
                   INNER JOIN evento ON inscribe.Evento_idEvento = evento.idEvento
                   WHERE evento.idEvento = ? AND inscribe.estado = 'reserva'`;
    conexion.query(query, [idEvento], (error, results) => {
        if (error) throw error;
        res.json(results);
    });
...
//Devuelve todos los eventos cotrolados por un controlador
router.get('/eventosControlador/:Usuario_ci', (req, res) => {
    const { Usuario_ci } = req.params;
    const query = `SELECT evento.*
                   FROM evento
                   INNER JOIN controla ON evento.idEvento = controla.Evento_idEvento
                   INNER JOIN controlador ON controla.Controlador_userCont = controlador.userCont
                   WHERE controlador.Usuario_ci = ?`;
...
//Obtener un usuario
router.get('/usuario/:ci', (req, res) => {
    const { ci } = req.params
    let sql = 'select * from usuario where ci = ?'
    conexion.query(sql, [ci], (err, rows, fields) => {
        if (err) throw err;
        else {
            res.json(rows)
        }
...
router.get('/logo', (req, res) => {
    let sql = 'SELECT * FROM informacion ORDER BY idLogo DESC LIMIT 1'
    conexion.query(sql, (err, rows, fields) => {
        if (err) throw err;
        else {
            res.json(rows)
...
//Modificar usuario
router.put('/modUser/:ci', (req, res) => {
    const { ci } = req.params
    const { nombre, apellido, tituloProf, telefono, correo, pass, nacionalidad } = req.body
...
router.put('/modInscripcion/:nroIncr', (req, res) => {
    const { nroIncr } = req.params
    const { asistencia, estado, fechaInscripcion } = req.body
    let sql = `update inscribe set
                asistencia = '${asistencia}',
                estado = '${estado}',
                fechaInscripcion = '${fechaInscripcion}'
                where nroIncr ='${nroIncr}'`
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
        else {
            res.json({ status: 'Inscribe editado' })
        }
    })
})
...
router.put('/editarPerfil/:ci', (req, res) => {
    const { ci } = req.params
    const {  nombre, apellido, tituloProf, telefono, correo, pass, nacionalidad, foto } = req.body
    let sql = `update usuario set
                nombre = '${nombre}',
                apellido = '${apellido}',
                tituloProf = '${tituloProf}',
    ...
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
            ...
            res.json({ status: 'Uusario editado' })
        }
    })
})
...
router.delete('/elimTextIni/:idInicioP', (req, res) => {
    const idInicioP = req.params.idInicioP;
    let sql = `DELETE FROM iniciobaner WHERE idInicioP = ${idInicioP}`;
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
        else {
            res.json({ status: 'texto eliminado' });
        }
    });
});
//Elimina usuario de ci x

router.delete('/elimUser/:ci', (req, res) => {
    const ci = req.params.ci;
    let sql = `DELETE FROM usuario WHERE ci = ${ci}`;
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
        else {
            res.json({ status: 'texto eliminado' });
...
router.delete('/eliminarInscribe/:nroIncr', (req, res) => {
    const nroIncr = req.params.nroIncr;
    let sql = `DELETE FROM inscribe WHERE nroIncr = ${nroIncr}`;
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
        else {
            res.json({ status: 'inscribe eliminado' });
        }
    });
});
router.delete('/eliminarActividad/:idActividad', (req, res) => {
    const idActividad = req.params.idActividad;
    let sql = `DELETE FROM actividad WHERE idActividad = ${idActividad}`;
    conexion.query(sql, (err, rows, field) => {
        if (err) throw err;
        else {
            res.json({ status: 'inscribe eliminado' });
